...
it("parse ID strings", () => {
  ...
  expect(parsed.date.getTime()).to.equal(
    new Date("2022-02-11T17:17:38.000Z").getTime()
  );

  expect(parsed.timestamp).to.equal(244599858);

  expect(parsed.payload.toString("hex")).to.equal(
    "76e8fbc01cd81a5b6120178d5d76e9fe"
  );

  expect(parsed.raw.toString("hex")).to.equal(
    "0e944c3276e8fbc01cd81a5b6120178d5d76e9fe"
  );
...
it("validate id string", () => {
  expect(ID.validateIdString("player", "player_24yPHHYrYySQDhHsqEAFjCtuS9d")).to
    .be.true;

  expect(ID.validateIdString("person", "player_24yPHHYrYySQDhHsqEAFjCtuS9d")).to
    .be.false;
