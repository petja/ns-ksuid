...
export default class NS_KSUID<NS extends string = string> {
  ...
  compare(other: NS_KSUID<NS>) {
    if (this.namespace !== other.namespace)
      throw new TypeError("Cannot compare IDs from different namespaces");
  ...
  static randomSync<NS extends string>(
    namespace: NS,
  ...
  static async random<NS extends string>(
    namespace: NS,
    timeInMs?: Date | number
  ): Promise<NS_KSUID<NS>> {
  ...
  static fromParts<NS extends string>(
    namespace: NS,
    timeInMs: number,
    payload: Buffer
  ): NS_KSUID<NS> {
  ...
  static validateIdString<NS extends string>(
    namespace: NS,
    str: string
  ): str is IdString<NS> {
